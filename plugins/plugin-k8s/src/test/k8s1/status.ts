...
const doHeadless = (ctx: common.ISuite, impl: CLI) => {
  ...
  it('should create sample pod from local file', () => {
    return impl.do(`kubectl create -f ${ROOT}/data/k8s/headless/pod.yaml -n ${ns}`, ctx.app)
  ...
  it('should delete the new pod by yaml', () => {
    return impl.do(`kubectl delete -f ${ROOT}/data/k8s/headless/pod.yaml  -n ${ns}`, ctx.app)
  ...
  it('should NOT list the new pod via the status command', () => {
    return impl.do(`k status pods -n ${ns}`, ctx.app)
