...
export default async (commandTree, wsk) => {
  ...
  const rmHelper = type => entities => {
    if (!entities || entities.length === 0) {
      ...
      return Promise.all(entities.map(_ => repl.qexec(`${type} rimraf -r -q ${repl.encodeComponent(_)}`, undefined, undefined, { raw: true })))
    }
  }
   ...
   *
   */
  const deletePackageAndContents = pckage => new Promise((resolve, reject) => {
    repl.qexec(`package get "${pckage}"`)
      ...
      .then(removedSoFar => {
        ...
        //
        const tryDelete = () => repl.qexec(`package delete "${pckage}"`)
  ...
  const rm = type => ({ block, nextBlock, argv: fullArgv, execOptions }) => {
    ...
    return glob(type, toBeDeletedList).then(toBeDeleted => Promise.all(toBeDeleted.map(match => {
      ...
      if (options.recursive && type === 'actions') {
        ...
        //                         ^^^^^^ delete this, too
        //
        return repl.qexec(`action get "${arg}"`, block)
          .then(action => {
