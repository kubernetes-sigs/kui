...
export default (commandTree: CommandRegistrar) => {
  ...
  const render = (tab: Tab, input: string, options, execOptions, mode) =>
    new Promise((resolve, reject) => {
      ...
      if (extension === 'json' || extension === 'ast') {
        ...
        try {
          fsmPromise = compileUtil.sourceToComposition({
            inputFile: input,
      ...
      const formatForUser = (mode: string) => async (composition: CompositionWithCode) => {
        ...
        const { view, controller } = await wskflowUtil.wskflow(tab, visualize, {
          ast,
          name,
          viewOptions,
