...
const inputSelector = `${searchBarSelector} input`
const buttonsSelector = `${searchBarSelector} .pf-c-text-input-group__utilities`
const countSelector = `${buttonsSelector} .pf-c-badge.pf-m-read`
const previousButton = `${buttonsSelector} button[aria-label="Previous"]`
...
Common.localDescribe('Text search', function(this: Common.ISuite) {
  ...
  */
  it('should open the search bar when cmd+f is pressed', async () => {
    ...
    await this.app.client.$(searchBarSelector).then(_ => _.waitForDisplayed({ timeout: CLI.waitTimeout }))
  ...
  it('should not close the search bar if pressing esc outside of search bar', async () => {
    ...
    await this.app.client.$(searchBarSelector).then(_ => _.waitForDisplayed({ timeout: 3000 }))
  })
  ...
  it('should focus on search bar when search bar is pressed', async () => {
    ...
    await this.app.client.waitUntil(
      async () => {
        return await this.app.client.$(inputSelector).then(_ => _.isFocused())
      },
      { timeout: 3000 }
    )
  })
  ...
  it('should close the search bar via ctrl+f', async () => {
    await this.app.client.keys([Keys.ctrlOrMeta, 'F'])
    await this.app.client.$(searchBarSelector).then(_ => _.waitForDisplayed({ timeout: 3000, reverse: true }))
  })
  ...
  */
  const type = async (text: string) => {
    ...
    await this.app.client.$(inputSelector).then(_ => _.setValue(''))
    // pasting the input text into the search bar input field
    await this.app.client.$(inputSelector).then(_ => _.setValue(text))
    // making sure the word in the input field is the same word we want to search for
    await this.app.client.waitUntil(
      async () => {
        const actualText = await this.app.client.$(inputSelector).then(_ => _.getValue())
  ...
  const findMatch = (typeText: string, searchFoundText: string) => {
    it(`should find ${searchFoundText} matches for ${typeText}`, async () => {
      try {
        ...
        await this.app.client.waitUntil(
          async () => {
            await this.app.client.$(inputSelector).then(_ => _.waitForExist({ timeout: 3000 }))
            const txt = await this.app.client.$(countSelector).then(_ => _.getText())
            const totalMatches = txt.substring(txt.indexOf('/') + 1)
            await this.app.client.$(closeButtonSelector).then(_ => _.click())
  ...
  */
  it('should not display results counter when search bar is opened and input field is empty', async () => {
    ...
    await this.app.client.$(countSelector).then(_ => _.waitForDisplayed({ timeout: 3000, reverse: true }))
  })
  ...
  it('should not display navigation arrows when search bar is opened and input field is empty', async () => {
    ...
    await this.app.client.$(previousButton).then(_ => _.waitForDisplayed({ timeout: 3000, reverse: true }))
    await this.app.client.$(nextButton).then(_ => _.waitForDisplayed({ timeout: 3000, reverse: true }))
  })
  ...
  it('should not display close button when search bar is opened and input field is empty', async () => {
    ...
    await this.app.client.$(closeButtonSelector).then(_ => _.waitForDisplayed({ timeout: 3000, reverse: true }))
  })
  ...
  it('should display results counter when search bar input field has text', async () => {
    ...
    await this.app.client.$(countSelector).then(_ => _.waitForDisplayed({ timeout: 3000 }))
  })

  it('should display navigation arrows when search bar input field has text', async () => {
    // make sure navigation arrows are displayed
    await this.app.client.$(previousButton).then(_ => _.waitForDisplayed({ timeout: 3000 }))
    await this.app.client.$(nextButton).then(_ => _.waitForDisplayed({ timeout: 3000 }))
  })

  it('should display close button when search bar input field has text', async () => {
    // make sure close button is is displayed
    await this.app.client.$(closeButtonSelector).then(_ => _.waitForDisplayed({ timeout: 3000 }))
  })
  ...
  */
  it('should increase result count when "NEXT" navigation arrow is pressed', async () => {
    ...
    const resultCount = await this.app.client.$(countSelector).then(_ => _.getText())
    const oldResult = resultCount.substring(0, resultCount.indexOf('/'))
    // clicking the NEXT arrow once
    await this.app.client.$(nextButton).then(_ => _.click())
    // getting the new result count
    const newCount = await this.app.client.$(countSelector).then(_ => _.getText())
  ...
  it('should decrease result count when "PREVIOUS" navigation arrow is pressed', async () => {
    ...
    await this.app.client.$(previousButton).then(_ => _.click())
    // getting the new result count
    const newCount = await this.app.client.$(countSelector).then(_ => _.getText())
    const curResultCount = newCount.substring(0, newCount.indexOf('/'))
  ...
  it('should do nothing if on last result and "NEXT" navigation arrow is pressed', async () => {
    ...
    while (iter !== totalMatches) {
      ...
      await this.app.client.$(nextButton).then(_ => _.click())
      const tmp = await this.app.client.$(countSelector).then(_ => _.getText())
  ...
  */
  it('should close the search bar if clicking the close button', async () => {
    ...
    await this.app.client.waitUntil(async () => {
      ...
      const textInSearchBar = await this.app.client.$(inputSelector).then(_ => _.getValue())
