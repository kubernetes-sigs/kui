...
describe('Confirm dialog', function(this: common.ISuite) {
  ...
  it('should fail to exec if the command is missing', () =>
    cli.do(`confirm`, this.app).then(cli.expectError(0, 'Missing command to execute')))

  it('should not do anything if user cancel', () => {
    return cli
      .do(`confirm --then "echo hello"`, this.app)
  ...
  it('should execute if user confirm', () => {
    return cli
      .do(`confirm --then "echo hello"`, this.app)
      .then(() => this.app.client.waitForExist('#confirm-dialog'))
