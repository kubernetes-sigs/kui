  ...
  commandTree.listen(
    ...
    ({ tab, parsedOptions }) =>
      ...
      new Promise(async (resolve, reject) => {
        const message = parsedOptions.asking || 'Are you sure?'
        const command = parsedOptions.then
        if (!command) {
          return reject(new Error('Missing command to execute'))
        }
        ...
        const confirm = () => {
          ...
          headerText.innerText = 'Please confirm'
          ...
          confirmMessage.innerHTML = `<p>You want to execuate the command <br> <strong>${command}</strong> <br> ${message} </p>`
          ...
          continueButton.innerText = "Yes, I'm sure"
        ...
        resolve('Confirmation modal pop up')
      }),
