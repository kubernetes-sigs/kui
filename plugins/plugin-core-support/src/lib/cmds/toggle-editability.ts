...
interface EditOptions extends ParsedOptions {
  'new-window': boolean
  'status-stripe': import('@kui-shell/core/mdist/api/Events').StatusStripeChangeEvent['type']
  current?: boolean
  c?: boolean
}
...
export default function (registrar: Registrar) {
  ...
  registrar.listen<KResponse, EditOptions>(
    '/tab/edit/toggle',
    async ({ tab, argvNoOptions, parsedOptions }) => {
      const { isOfflineClient, isReadOnlyClient } = await import('@kui-shell/core/mdist/api/Client')
