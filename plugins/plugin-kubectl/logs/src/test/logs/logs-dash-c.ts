...
describe(`kubectl container logs ${process.env.MOCHA_RUN_TARGET || ''}`, function(this: Common.ISuite) {
  ...
  const containerName1 = 'a'
  const containerName2 = 'b'
  ...
  it('should show logs tab', async () => {
    try {
      ...
      return this.app.client.waitUntil(async () => {
        ...
        return text.includes(containerName1) && text.includes(containerName2)
      })
    } catch (err) {
      return Common.oops(this, true)(err)
    }
  ...
  const switchContainer = (container: string, showInLog: string, notShowInLog: string) => {
    it(`should switch to container ${container}`, async () => {
      try {
        ...
        await this.app.client.waitForVisible(`button[data-mode="${container}"]`)
        await this.app.client.click(`button[data-mode="${container}"]`)
    ...
    it(`should show ${showInLog} in log output`, async () => {
      try {
        await this.app.client.waitUntil(async () => {
          const text = await this.app.client.getText(`${Selectors.SIDECAR} .kui--sidecar-text-content`)
          return text.includes(showInLog) // the logs of the container will only have the container name
        })
      } catch (err) {
        return Common.oops(this, true)(err)
      }
    ...
    it(`should not show ${notShowInLog} in log output`, async () => {
      try {
        await this.app.client.waitUntil(async () => {
          const text = await this.app.client.getText(`${Selectors.SIDECAR} .kui--sidecar-text-content`)
          return !text.includes(notShowInLog) // the logs of the container will only have the container name
        })
      } catch (err) {
        return Common.oops(this, true)(err)
      }
