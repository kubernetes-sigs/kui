...
const root = path.dirname(require.resolve('@kui-plugin-src/apache-composer/package.json'))
const src = app => fs.readFileSync(path.join(root, '@demos/', `${app}.js`)).toString()
      ...
      'components': [
        {
          ...
          'function': { 'exec': {
            'kind': 'nodejs:default',
            'code': "args => ({ text: Buffer.from(args.params.str, 'base64').toString() })"
          } }
        }
      ]
    },
