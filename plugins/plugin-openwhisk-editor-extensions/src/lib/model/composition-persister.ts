...
const debug = Debug(
  'plugins/openwhisk-editor-extensions/model/composition-persister'
)
...
export const handleParseError = (err, filepath, editor) => {
  if (err.statusCode === 'ENOPARSE') {
    ...
    const match =
      err.message.match(pattern1) || err.message.match(pattern2) || match3
      ...
      const decorations = [
        {
          ...
          options: {
            ...
            glyphMarginClassName:
              'editor parse-error-gutter-marker parse_error_decoration',
...
export const persister = {
  getCode: action =>
    new Promise((resolve, reject) => {
      ...
      if (codeAnno) {
        ...
        const localCodePath = action.annotations.find(
          ({ key }) => key === 'file'
        )
        if (localCodePath) {
          ...
          const err = new Error(
            'Your composition does not have an assocated source file'
          )
          err['code'] = 406
          reject(err)
        }
  ...
  save: (app, editor) =>
    new Promise((resolve, reject) => {
      ...
      tmp.file(
        { prefix: 'shell-', postfix: extension(app.exec.kind) },
          ...
          if (err) {
            ...
            fs.write(fd, app.exec.code, err => {
              if (err) {
                ...
                return pexec(
                  `app update ${app.name} ${filepath} --kind ${app.exec.kind}`
                )
                  .then(app => {
                    cleanup()
                    resolve(app)
