...
let electron
let app
async function initGraphics (command = [], subwindowPlease?, subwindowPrefs?: SubwindowPrefs) {
  ...
  try {
    ...
    const maybeSpawnGraphics = async () => {
      ...
      if (app.graphics) {
        ...
        console.log('Graphical components are not installed.'.red)
        process.exit(126)
      }
    }
    ...
    try {
      ...
      if (!_location || name !== '@ibm-functions/shell') {
...
async function initHeadless (force = false) {
...
const getCommand = argv => {
  ...
  const isShell = argv.find(_ => _ === 'shell') || process.env.RUNNING_SHELL_TEST
  argv = argv.filter(_ => _ !== '--ui')
 ...
 *
 */
const forceUI = process.argv.find(arg => arg === '--ui')
const isShell = process.argv.find(arg => arg === 'shell')
const kuiShell = forceUI || isShell
const isRunningHeadless = process.env.KUI_HEADLESS && !kuiShell
if (!isRunningHeadless) {
  // then spawn the electron graphics
  debug('shortcut to graphics')
  const { argv, subwindowPlease, subwindowPrefs } = getCommand(process.argv)
  initGraphics(argv,
    process.env.subwindowPlease || subwindowPlease,
    process.env.subwindowPrefs
      ? typeof process.env.subwindowPrefs === 'string' ? JSON.parse(process.env.subwindowPrefs) : process.env.subwindowPrefs
      : subwindowPrefs)
} else {
...
function createWindow (noHeadless = false, executeThisArgvPlease?, subwindowPlease?, subwindowPrefs?) {
  ...
  if (!electron) {
    ...
    promise = initGraphics(['--'].concat(executeThisArgvPlease), subwindowPlease, subwindowPrefs)
      .then(x => {
        electron = require('electron')
      }).catch(err => {
        // headless
