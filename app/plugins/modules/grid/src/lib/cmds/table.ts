...
import { addNameToSidecarHeader } from '../../../../../../build/webapp/views/sidecar'
...
const tableModes = choicesArray.concat([
...
const _drawTable = (options, header: IHeader, content: Element, groupData, eventBus, uuid: string, sorter = statDataSorter(defaultTop), sortDir = +1) => {
  ...
  const draw = ({ bottom: stat25, top: stat75, redraw = false, showOutliers = false }) => {
    ...
    groups.forEach((group, idx) => {
      ...
      const splitOptions = options.split ? `--split${options.split === true ? '' : ' "' + options.split + '"'} --key "${group.groupKey}"` : ''
