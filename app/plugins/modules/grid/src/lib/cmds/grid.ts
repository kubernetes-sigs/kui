...
import { addNameToSidecarHeader, showCustom } from '../../../../../../build/webapp/views/sidecar'
...
const drawGrid = (options: IOptions, header: IHeader, uuid: string, redraw = false) => (activations: Array<Object>) => {
  ...
  const asTimeline = { mode: 'as-timeline',
...
const _drawGrid = (options, { leftHeader, rightHeader }, content, groupData, sorter = countSorter, sortDir = +1, redraw) => {
  ...
  groups.forEach((group, groupIdx) => {
    ...
    if (!redraw) {
      ...
      const vws = zoomLevelForDisplay === 0 ? 2.75 : zoomLevelForDisplay === -1 ? 1.25 : zoomLevelForDisplay === 1 ? 3 : zoomLevelForDisplay === 2 ? 4 : 1
