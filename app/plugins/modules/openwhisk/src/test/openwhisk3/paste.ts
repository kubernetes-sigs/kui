...
describe('Execute commands via paste', function (this: ISuite) {
  ...
  it('should get the action created by the first line', () => cli.do(`action get ${actionName4}`, this.app)
    .then(cli.expectJustOK)
    .then(sidecar.expectOpen)
    .then(sidecar.expectShowing(actionName4))
    .catch(common.oops(this)))

  it('should paste three lines without trailing newline', () => Promise.resolve(this.app.electron.clipboard.writeText(`let ${actionName6} = x=>x\nlet ${actionName7} = x=>x\n\n\n\n   \n   \n\nlet ${actionName8} = x=>x`))
    .then(() => this.app.client.execute(() => document.execCommand('paste')))
    .then(() => cli.do(ui.keys.ENTER, this.app))
    .then(cli.expectJustOK)
    .then(sidecar.expectOpen)
  ...
  it('should get the action created by the first line', () => cli.do(`action get ${actionName6}`, this.app)
    .then(cli.expectJustOK)
    .then(sidecar.expectOpen)
    .then(sidecar.expectShowing(actionName6))
    .catch(common.oops(this)))
  it('should get the action created by the second line', () => cli.do(`action get ${actionName7}`, this.app)
    .then(cli.expectJustOK)
    .then(sidecar.expectOpen)
