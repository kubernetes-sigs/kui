...
function theme {
    # filesystem icons
    ICON_MAC="$THEME"/$(cd $THEME && node -e 'console.log(require("./config.d/icon").filesystem.darwin)')
    ICON_WIN32="$THEME"/$(cd $THEME && node -e 'console.log(require("./config.d/icon").filesystem.win32)')
    ICON_LINUX="$THEME"/$(cd $THEME && node -e 'console.log(require("./config.d/icon").filesystem.linux)')
