...
if [ -d node_modules/@kui-shell/client/notebooks ]; then
    ...
    (echo -n "["; (cd node_modules/\@kui-shell/client/notebooks && find -L . \( -name '*.md' -o -name '*.py' -o -name '*.yaml' -o -name '*.yml' -o -name '*.txt' -o -name '*.json' \) -print) | sed 's/\.\///' | xargs -I{} -n1 echo -n '"{}",'; echo -n "]") | sed 's/\,]/]/' > "$TGT"
fi
