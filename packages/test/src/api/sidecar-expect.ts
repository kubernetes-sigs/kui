...
export const badge =
  (title: string, css?: string, absent = false) =>
...
export const toolbarText =
  (expect: { type: string; text: string; exact?: boolean; timeout?: number }) => async (res: AppAndCount) => {
    await expectText(
      res.app,
      expect.text,
      expect.exact || false,
...
const show =
  (expected: string, selector: string) =>
...
export const modes =
  (expected: { mode: string; label?: string; dafaultMode?: boolean }[]) => async (res: AppAndCount) =>
    Promise.all(
...
export const textPlainContentFromMonaco =
  (expectedText: string, exact = true) =>
  async (res: AppAndCount) => {
    let idx = 0
    await res.app.client.waitUntil(
      async () => {
        ...
        if (++idx > 5) {
          console.error(
            `still waiting for plain text from monaco actualText=${actualText} expectedText=${expectedText}`
          )
        }
  ...
  async (res: AppAndCount): Promise<AppAndCount> => {
    await res.app.client.waitUntil(
      async () => {
        ...
        if (!which) {
          which = (await res.app.client.$(sidecarSelector).then(_ => _.getAttribute('data-view'))) as
            | 'leftnav'
            | 'topnav'
        }
        const titleSelector =
